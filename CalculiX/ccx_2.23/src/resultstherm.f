...
!     
     ...
     &     dtime,time,ttime,plkcon,nplkcon,xstateini,xstiff,xstate,
     &     npmat_,matname,mi,ncmat_,nstate_,cocon,ncocon,
     ...
     &     iponoeln,inoeln,network,ipobody,xbody,ibody,thicke)
!     
...
!     
     ...
     &     ncocon(2,*),iflag,nshcon(*),istep,iinc,mt,mattyp,kk,
     ...
     &     ilayer,nlayer,mint2d,nopes,ki,kl
!     
      real*8 co(3,*),v(0:mi(2),*),shp(4,20),reltime,thickness,
     &     xl(3,20),vl(0:mi(2),20),elcon(0:ncmat_,ntmat_,*),xsj2(3),
     &     rhcon(0:1,ntmat_,*),qfx(3,mi(1),*),orab(7,*),xl2(3,8),
     &     rho,fn(0:mi(2),*),tnl(19),timeend(2),q(0:mi(2),20),
     &     vkl(0:3,3),t0(*),vold(0:mi(2),*),coefmpc(*),xs2(3,7),
     ...
     &     xbody(7,*),tlayer(4),dlayer(4),xlayer(mi(3),4),shp2(7,8),
     &     thicke(mi(3),*)
!     
      include "gauss.f"
!     
...
!     
        ...
        elseif(lakonl(4:4).eq.'6') then
          nope=6
        elseif((lakonl(1:2).eq.'ES').and.(lakonl(7:7).ne.'A')) then
!     
...
!     
     ...
     &         (lakonl(4:6).eq.'20R')) then
          if(lakonl(6:7).eq.'RA') then
            mint3d=4
          elseif(lakonl(7:8).eq.'LC') then
            mint3d=8*nlayer
          else
            mint3d=8
          endif
...
!     
     ...
     &         (lakonl(4:5).eq.'8I')) then
          ...
          elseif(lakonl(4:6).eq.'20 ') then
            nopered=20
            call lintemp_th1(vold,konl,nopered,jj,t1lold,mi)
            call lintemp_th1(v,konl,nopered,jj,t1l,mi)
